---
type: aku
aku_class: concept
id: aku-funnel-corto-vs-largo-concept
statement: >
  El funnel corto vs. largo distingue la complejidad del proceso de venta: un
  funnel corto convierte la visita en compra de forma directa cuando la decisión
  es simple y barata (p. ej. una cartera de 10 € en una tienda online); un funnel
  largo es necesario cuando la decisión es compleja o cara (p. ej. un servicio de
  3.000 €/mes) y requiere cadenas de emails, información, resolver dudas y
  llamadas; implica que ninguno es mejor que el otro y que el funnel se optimiza
  según los objetivos del negocio.
origin: "The Power MBA — Módulo 5.3 «Visión estratégica»"
domain: [marketing-digital, funnel, ventas, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related: [aku-power-funnel-concept, aku-lead-nurturing-concept, aku-nurturing-segun-complejidad-claim]

sources:
  - raw/cursos/power-mba/5-3-visi-on-estrat-egica/5-3-visi-on-estrat-egica.md

created: 2026-06-03
updated: 2026-06-03
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-power-funnel-concept]] · [[aku-lead-nurturing-concept]] · [[aku-nurturing-segun-complejidad-claim]]
