---
type: aku
aku_class: claim
id: aku-callout-cocktail-party-claim
statement: >
  Que la gente note el anuncio es, con diferencia, lo más importante (Ogilvy: el
  headline gasta el 80% del presupuesto): el call-out aprovecha el «cocktail
  party effect» —un estímulo específico atraviesa el ruido— y debe ser lo
  bastante específico para atraer al cliente ideal y lo bastante amplio para
  captar a tantos como sea posible; los primeros ~5 segundos son la parte más
  testeada y enfocarse en ellos multiplicó por 20 la eficacia.
origin: "Alex Hormozi — $100M Leads"
domain: [marketing-digital, publicidad, copywriting, hormozi]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-anuncio-callout-value-cta-concept
    - aku-formulas-titulos-potentes-concept
    - aku-copy-incluir-titular-claim
    - aku-tu-nombre-capta-la-atencion-cocktail-party-claim
    - aku-anadir-elementos-de-la-atencion-top-down-del-espectador-claim

sources:
  - raw/libros/hormozi/100m-leads/100m-leads.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-anuncio-callout-value-cta-concept]] · [[aku-formulas-titulos-potentes-concept]] · [[aku-copy-incluir-titular-claim]] · [[aku-tu-nombre-capta-la-atencion-cocktail-party-claim]] · [[aku-anadir-elementos-de-la-atencion-top-down-del-espectador-claim]]
