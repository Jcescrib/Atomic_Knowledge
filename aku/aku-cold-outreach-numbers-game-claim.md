---
type: aku
aku_class: claim
id: aku-cold-outreach-numbers-game-claim
statement: >
  El cold outreach es un juego de números fiable: una cierta cantidad de inputs
  produce una cierta cantidad de respuestas, por lo que para conseguir más solo
  hay que hacer más (automatizar la entrega y la distribución del mensaje); una
  vez conocida la ecuación «X contactos → Y clientes», se reduce a resolver para
  X.
origin: "Alex Hormozi — $100M Leads"
domain: [marketing-digital, captacion, outbound, hormozi]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-cold-outreach-concept
    - aku-follow-up-volumen-claim
    - aku-herramientas-growth-hacking-concept
    - aku-cold-outreach-ventajas-claim
    - aku-cold-outreach-benchmarks-method
    - aku-construir-lista-tres-vias-concept
    - aku-cold-personalizar-como-warm-claim
    - aku-big-fast-value-claim
    - aku-volumen-cold-tres-palancas-concept
    - aku-asignar-10-20-tech-nueva-claim

sources:
  - raw/libros/hormozi/100m-leads/100m-leads.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-cold-outreach-concept]] · [[aku-follow-up-volumen-claim]] · [[aku-herramientas-growth-hacking-concept]] · [[aku-cold-outreach-ventajas-claim]] · [[aku-cold-outreach-benchmarks-method]] · [[aku-construir-lista-tres-vias-concept]] · [[aku-cold-personalizar-como-warm-claim]] · [[aku-big-fast-value-claim]] · [[aku-volumen-cold-tres-palancas-concept]] · [[aku-asignar-10-20-tech-nueva-claim]]
