---
type: aku
aku_class: concept
id: aku-recomendacion-vs-viralizacion-concept
statement: >
  La recomendación y la viralización son estrategias para que los propios clientes
  atraigan nuevos clientes; se distinguen por el mecanismo: la recomendación se
  apoya en la voluntad del cliente de compartir información del producto
  (programas referral tipo «trae un amigo y os regalamos 50 €»), mientras que la
  viralización ocurre cuando el producto se da a conocer solo por usarlo (la marca
  de agua de TikTok al compartir en Instagram, la firma de TweetDeck); implica que
  ambas atraen leads y reducen el CAC a un coste cercano a cero.
origin: "The Power MBA — Módulo 5.8.1 «Recomendación y viralización»"
domain: [marketing-digital, recomendacion, viralidad, captacion, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: [aku-boca-a-boca-supera-campana-claim, aku-recomendacion-depende-producto-claim]
  constrained_by: [aku-viralizacion-coste-cercano-cero-claim]
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related: [aku-coeficiente-viralidad-concept, aku-cac-concept]

sources:
  - raw/cursos/power-mba/5-8-1-conceptos-clave-recomendacion-y-viralizacion/5-8-1-conceptos-clave-recomendacion-y-viralizacion.md

created: 2026-06-03
updated: 2026-06-03
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-boca-a-boca-supera-campana-claim]] · [[aku-recomendacion-depende-producto-claim]]
**constrained_by** ← [[aku-viralizacion-coste-cercano-cero-claim]]
**related** ↔ [[aku-coeficiente-viralidad-concept]] · [[aku-cac-concept]]
