---
type: aku
aku_class: claim
id: aku-no-relajarse-hasta-completar-la-mision-claim
statement: >
  La tendencia a relajarse en cuanto se cumple el objetivo primario de una
  misión es peligrosa: no puedes relajarte hasta que la misión ENTERA está
  completa, porque el enemigo acecha el momento de debilidad (el exhalar, bajar
  el arma); hay que ser implacable (relentless) —nunca está terminado, siempre
  hay otra misión, tarea u objetivo—.
origin: "Jocko Willink, Discipline Equals Freedom: Field Manual (2017)"
domain: [liderazgo, jocko, disciplina, implacabilidad]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports:
    - aku-disciplina-raiz-de-toda-buena-cualidad-concept
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-nunca-complacencia-subestimar-enemigo-claim
    - aku-destroyer-mode-emocion-y-logica-concept
    - aku-pelear-hasta-el-final-nada-que-perder-claim

sources:
  - raw/libros/jocko/discipline-equals-freedom/discipline-equals-freedom.md

created: 2026-06-05
updated: 2026-06-05
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-disciplina-raiz-de-toda-buena-cualidad-concept]]

**related** ↔ [[aku-nunca-complacencia-subestimar-enemigo-claim]] · [[aku-destroyer-mode-emocion-y-logica-concept]] · [[aku-pelear-hasta-el-final-nada-que-perder-claim]]
