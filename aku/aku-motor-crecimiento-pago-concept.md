---
type: aku
aku_class: concept
id: aku-motor-crecimiento-pago-concept
statement: >
  El motor de crecimiento de pago adquiere clientes mediante publicidad
  pagada; incluye negocios donde CAC es la métrica clave de eficiencia y el
  margen CLTV − CAC se reinvierte en marketing para alimentar el ciclo de
  captación; excluye negocios sin canales de adquisición pagados o con unit
  economics negativos (CLTV < CAC); implica que el ritmo de crecimiento está
  acotado por el margen unitario y el CAC payback (cuanto menor el payback,
  antes se puede reinvertir el margen en marketing y crecer sin financiación
  externa adicional). Ejemplos: Hawkers, e-commerce de productos.
origin: "The Power MBA — Módulo 2.2.1 «Tipos de motores de crecimiento»"
domain: [growth, business-model, paid-acquisition, unit-economics, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-motor-crecimiento-concept
    - aku-motor-crecimiento-viral-concept
    - aku-motor-crecimiento-sticky-concept
    - aku-cltv-concept
    - aku-cac-concept
    - aku-cltv-minus-cac-concept
    - aku-cac-payback-concept

sources:
  - "raw/2-2-1-tiposmotorescrecimiento/2-2-1-tiposmotorescrecimiento.md"

created: 2026-06-03
updated: 2026-06-03
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-motor-crecimiento-concept]] · [[aku-motor-crecimiento-viral-concept]] · [[aku-motor-crecimiento-sticky-concept]] · [[aku-cltv-concept]] · [[aku-cac-concept]] · [[aku-cltv-minus-cac-concept]] · [[aku-cac-payback-concept]]
