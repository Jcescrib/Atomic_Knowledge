---
type: aku
aku_class: method
id: aku-corregir-hands-off-method
statement: >
  Para corregir un liderazgo demasiado hands-off: dar guía clara (misión,
  objetivo y end state de forma simple, clara y concisa); definir los límites y
  qué hacer al toparse con ellos; cuando hay esfuerzos solapados, decidir e
  implementar el curso de acción elegido; educar al equipo sobre los esfuerzos
  de otros equipos para desconflictar; y si hay «demasiados entrenadores y pocos
  jugadores», asignar y delinear claramente la cadena de mando, roles y
  autoridad.
origin: "Jocko Willink & Leif Babin, The Dichotomy of Leadership (2018)"
domain: [liderazgo, jocko, mando, correccion]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports:
    - aku-balance-ownership-decentralized-command-concept
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-sintomas-de-hands-off-method
    - aku-micromanagement-mata-iniciativa-claim
    - aku-sintomas-de-micromanagement-method
    - aku-corregir-micromanagement-method
    - aku-hands-off-laissez-faire-descoordina-claim
    - aku-asignar-lead-claro-evita-planes-divergentes-claim

sources:
  - raw/libros/jocko/dichotomy-of-leadership/dichotomy-of-leadership.md

created: 2026-06-05
updated: 2026-06-05
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-balance-ownership-decentralized-command-concept]]

**related** ↔ [[aku-sintomas-de-hands-off-method]] · [[aku-micromanagement-mata-iniciativa-claim]] · [[aku-sintomas-de-micromanagement-method]] · [[aku-corregir-micromanagement-method]] · [[aku-hands-off-laissez-faire-descoordina-claim]] · [[aku-asignar-lead-claro-evita-planes-divergentes-claim]]
