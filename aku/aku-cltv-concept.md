---
type: aku
aku_class: concept
id: aku-cltv-concept
statement: >
  El CLTV (Customer Lifetime Value) es el valor económico total que un único
  cliente genera a lo largo de toda la relación con el negocio; incluye los
  ingresos totales o el margen bruto atribuibles a ese cliente (preferiblemente
  margen bruto); excluye el coste de adquisición (que se contabiliza por
  separado como CAC); implica que la retención tiene valor monetario directo,
  que ninguna decisión de adquisición está completa sin una estimación de
  CLTV, y que el CLTV requerido depende del motor de crecimiento (alto en
  motores de pago, bajo aceptable en motores virales).
origin: "The Power MBA — Módulo 2.2 «Motores de crecimiento — principales métricas»"
domain: [growth, unit-economics, business-model, metrics, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports:
    - aku-cltv-subscription-formula
    - aku-cltv-transactional-formula
    - aku-cltv-cac-dual-optimization
    - aku-cac-payback-concept
    - aku-cltv-cac-ratio-concept
    - aku-cltv-minus-cac-concept
  constrained_by:
    - aku-cltv-gross-margin-over-revenue
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-cac-concept
    - aku-arpu-concept
    - aku-churn-rate-concept
    - aku-motor-crecimiento-pago-concept
    - aku-viral-bajo-cltv-compatible-claim
    - aku-metricas-accionables-concept
    - aku-palancas-marketing-digital-concept
    - aku-lifetime-concept
    - aku-ltgp-concept
    - aku-affiliate-ltgp-cac-method

sources:
  - "raw/cursos/power-mba/2-2-motores-de-crecimiento-proncipales-m-etricas/2-2-motores-de-crecimiento-proncipales-m-etricas.md"

created: 2026-06-03
updated: 2026-06-03
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-cltv-subscription-formula]] · [[aku-cltv-transactional-formula]] · [[aku-cltv-cac-dual-optimization]] · [[aku-cac-payback-concept]] · [[aku-cltv-cac-ratio-concept]] · [[aku-cltv-minus-cac-concept]]

**constrained_by** ← [[aku-cltv-gross-margin-over-revenue]]

**related** ↔ [[aku-cac-concept]] · [[aku-arpu-concept]] · [[aku-churn-rate-concept]] · [[aku-motor-crecimiento-pago-concept]] · [[aku-viral-bajo-cltv-compatible-claim]] · [[aku-metricas-accionables-concept]] · [[aku-palancas-marketing-digital-concept]] · [[aku-lifetime-concept]] · [[aku-ltgp-concept]] · [[aku-affiliate-ltgp-cac-method]]
