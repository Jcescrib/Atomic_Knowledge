---
type: aku
aku_class: concept
id: aku-factores-competitivos-concept
statement: >
  Los factores competitivos son las dimensiones alrededor de las cuales gira
  la competencia en un sector concreto y en las que las empresas invierten
  para ofrecer valor (precio, calidad, servicio, diseño, etc.); incluye los
  factores específicos del sector (e.g., restauración: localización,
  decoración, calidad de la comida, chef reconocido, ambiente; automoción:
  marca, motor, gama, diseño); excluye factores irrelevantes para los
  clientes del sector; implica que identificar correctamente los factores
  competitivos es prerequisito para dibujar la curva de valor y para tomar
  decisiones estratégicas de diferenciación o de océano azul.
origin: "The Power MBA — Módulo 2.6 «Cómo crear propuestas de valor disruptivas y buscar tu océano azul»"
domain: [strategy, blue-ocean, competitive-analysis, value-proposition, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports:
    - aku-curva-valor-concept
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-competencia-amplia-concept

sources:
  - "raw/cursos/power-mba/2-6-oceano-azul-como-crear-propuestas-de-valor-disruptivas-y-buscar-para-tu-oc-eano-azul-conceptos-clave/2-6-oceano-azul-como-crear-propuestas-de-valor-disruptivas-y-buscar-para-tu-oc-eano-azul-conceptos-clave.md"

created: 2026-06-03
updated: 2026-06-03
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-curva-valor-concept]]

**related** ↔ [[aku-competencia-amplia-concept]]
