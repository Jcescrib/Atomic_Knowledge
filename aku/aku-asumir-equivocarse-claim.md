---
type: aku
aku_class: claim
id: aku-asumir-equivocarse-claim
statement: >
  En contextos de alta incertidumbre, asumir explícitamente que algunas de
  las hipótesis fundamentales del modelo de negocio resultarán equivocadas
  produce mejores resultados que asumir que se sabe lo que va a pasar; la
  primera disposición conduce a diseñar experimentos para descubrir el
  error temprano (cuando corregirlo es barato), la segunda conduce a
  construir el producto completo basado en supuestos no validados y
  descubrir el error tarde (cuando ya se ha invertido tiempo y dinero).
origin: "The Power MBA — Módulo 3.0.1 «El método Lean Startup» (Eric Ries)"
domain: [lean-startup, methodology, decision-making, risk, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports:
    - aku-lean-startup-method-concept
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-no-dar-supuesto-hipotesis-claim

sources:
  - "raw/3-0-1-lean-startup-lean-startup-recursos/3-0-1-lean-startup-lean-startup-recursos.md"

created: 2026-06-03
updated: 2026-06-03
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-lean-startup-method-concept]]

**related** ↔ [[aku-no-dar-supuesto-hipotesis-claim]]
