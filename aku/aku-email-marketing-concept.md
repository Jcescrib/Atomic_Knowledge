---
type: aku
aku_class: concept
id: aku-email-marketing-concept
statement: >
  El email marketing consiste en atraer, nutrir y convertir clientes usando
  emails; es un canal de comunicación directo con los potenciales clientes y muy
  potente cuando se cuenta con una base de datos bien segmentada y de calidad;
  implica que, bien hecho, sigue funcionando muy bien pese a quienes lo dan por
  muerto.
origin: "The Power MBA — Módulo 5.4.1 «Atracción y branding»"
domain: [marketing-digital, canales, email, nurturing, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: [aku-canales-marketing-digital-concept]
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related: [aku-lead-nurturing-concept, aku-marketing-automation-concept]

sources:
  - raw/5-4-1-atracci-on-y-branding/5-4-1-atracci-on-y-branding.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-canales-marketing-digital-concept]]
**related** ↔ [[aku-lead-nurturing-concept]] · [[aku-marketing-automation-concept]]
