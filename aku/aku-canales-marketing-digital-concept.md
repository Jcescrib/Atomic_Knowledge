---
type: aku
aku_class: concept
id: aku-canales-marketing-digital-concept
statement: >
  Los canales de marketing digital son las distintas vías para llegar a los
  clientes en la atracción; incluye campañas offline, paid social (publicidad en
  redes), paid search/SEM (anuncios en buscadores: Google, Amazon, YouTube…),
  publicidad display y vídeo, programática (compra automatizada y segmentada de
  espacios), email marketing, influencer marketing, SEO y ASO (posicionamiento
  orgánico en buscadores y app stores), content marketing, redes sociales
  orgánicas, PR y plataformas de afiliados; implica que cada canal tiene su coste,
  velocidad, medición y capacidad de segmentación, y que conviene combinarlos
  según el negocio.
origin: "The Power MBA — Módulo 5.4.1 «Atracción y branding»"
domain: [marketing-digital, canales, atraccion, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: [aku-seo-aso-concept, aku-sem-paid-search-concept, aku-paid-social-concept, aku-red-display-concept, aku-programatica-concept, aku-email-marketing-concept, aku-marketing-afiliados-concept, aku-publicidad-offline-concept]
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related: [aku-canales-de-distribucion-concept, aku-atribucion-concept, aku-publicidad-nativa-concept, aku-marketing-influencers-concept, aku-inbound-marketing-concept, aku-content-marketing-concept]

sources:
  - raw/5-4-1-atracci-on-y-branding/5-4-1-atracci-on-y-branding.md

created: 2026-06-03
updated: 2026-06-03
status: active
status_note: ""
---

## Relaciones

**supported_by** ← [[aku-seo-aso-concept]] · [[aku-sem-paid-search-concept]] · [[aku-paid-social-concept]] · [[aku-red-display-concept]] · [[aku-programatica-concept]] · [[aku-email-marketing-concept]] · [[aku-marketing-afiliados-concept]] · [[aku-publicidad-offline-concept]]

**related** ↔ [[aku-canales-de-distribucion-concept]] · [[aku-atribucion-concept]] · [[aku-publicidad-nativa-concept]] · [[aku-marketing-influencers-concept]] · [[aku-inbound-marketing-concept]] · [[aku-content-marketing-concept]]
