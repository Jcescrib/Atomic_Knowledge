---
type: aku
aku_class: claim
id: aku-levantar-financiacion-como-ventas-claim
statement: >
  Levantar financiación es equivalente a las ventas y el marketing: hay que
  conocer al interlocutor (el inversor) y ser capaz de transmitir la propuesta de
  forma atractiva, fácil de entender y que genere interés; los emprendedores
  fracasan en captar fondos porque no saben cómo piensa un inversor y porque, aun
  teniendo buenos proyectos, no son capaces de «vender» su proyecto.
origin: "The Power MBA — Módulo 6.3 «Vender tu proyecto / relación con inversores»"
domain: [emprendimiento, financiacion, ventas, comunicacion]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related: [aku-vender-una-accion-concept, aku-leyes-persuasion-cialdini-concept]

sources:
  - raw/6-3-1-vender-tu-proyecto/6-3-1-vender-tu-proyecto.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-vender-una-accion-concept]] · [[aku-leyes-persuasion-cialdini-concept]]
