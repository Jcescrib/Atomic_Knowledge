---
type: aku
aku_class: concept
id: aku-reconnaissance-principle-concept
statement: >
  El Reconnaissance Principle consiste en creer que las respuestas que buscas
  existen y nunca subestimar el poder de la información nueva: ya conoces la mitad
  de lo que necesitas para tener éxito, y la otra mitad la descubres recopilando
  información del oponente antes y durante el encuentro (observación, sondeo de
  «touch and feel» para provocar y leer sus reacciones); cuanto más relajado estás,
  más información obtienes.
origin: "Rener Gracie & Paul Volponi — The 32 Principles (2023)"
domain: [jiu-jitsu, principios, jocko, reconnaissance-principle, informacion]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-kuzushi-principle-concept
    - aku-creation-principle-concept
    - aku-river-principle-concept
    - aku-prevention-principle-concept
    - aku-tension-principle-concept
    - aku-conducta-sigue-patrones-y-se-revela-claim
    - aku-ensenar-con-no-a-incorporar-feedback-claim
    - aku-presion-revela-debilidades-claim

sources:
  - raw/libros/jocko/32-principles/32-principles.md

created: 2026-06-05
updated: 2026-06-05
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-kuzushi-principle-concept]] · [[aku-creation-principle-concept]] · [[aku-river-principle-concept]] · [[aku-prevention-principle-concept]] · [[aku-tension-principle-concept]] · [[aku-conducta-sigue-patrones-y-se-revela-claim]] · [[aku-ensenar-con-no-a-incorporar-feedback-claim]] · [[aku-presion-revela-debilidades-claim]]
