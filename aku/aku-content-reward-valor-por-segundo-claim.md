---
type: aku
aku_class: claim
id: aku-content-reward-valor-por-segundo-claim
statement: >
  La recompensa (reward) de un contenido depende de cuántas veces recompensa por
  el tiempo que cuesta consumirlo («valor por segundo»): no existe contenido
  «demasiado largo», solo «demasiado aburrido»; quien decide si recompensó es la
  audiencia, no el creador, y la señal objetiva de éxito es que la audiencia
  crece.
origin: "Alex Hormozi — $100M Leads"
domain: [marketing-digital, contenido, hormozi]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-content-unit-concept
    - aku-content-hook-concept
    - aku-hook-topic-headline-format-concept
    - aku-content-retain-concept
    - aku-short-vs-long-content-claim

sources:
  - raw/libros/hormozi/100m-leads/100m-leads.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-content-unit-concept]] · [[aku-content-hook-concept]] · [[aku-hook-topic-headline-format-concept]] · [[aku-content-retain-concept]] · [[aku-short-vs-long-content-claim]]
