---
type: aku
aku_class: concept
id: aku-tests-personalidad-concept
statement: >
  Los tests de personalidad (MBTI, DISC, OCEAN, Gallup, Eneagrama, etc.) son
  herramientas que evalúan el tipo de personalidad para entendernos mejor a
  nosotros mismos y a los demás; incluye que ningún tipo es mejor que otro, solo
  con fortalezas y debilidades distintas; implica utilidad en la comunicación
  diaria, la formación de equipos, la gestión del estrés y el descubrimiento de
  puntos ciegos.
origin: "The Power MBA — Módulo 7.1 «Liderarse a uno mismo»"
domain: [leadership, autoconocimiento, psicologia, equipos, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: [aku-mbti-concept]
  constrained_by: [aku-test-personalidad-solo-preferencias-claim]
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related: [aku-liderazgo-concept, aku-descubrir-fortalezas-feedback-externo-concept]

sources:
  - raw/7-1-1-recursos-liderarse-a-uno-mismo-ok/7-1-1-recursos-liderarse-a-uno-mismo-ok.md
  - raw/08-01-02-herramientasdedesarrollopersonal/08-01-02-herramientasdedesarrollopersonal.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-mbti-concept]]
**constrained_by** ← [[aku-test-personalidad-solo-preferencias-claim]]
**related** ↔ [[aku-liderazgo-concept]] · [[aku-descubrir-fortalezas-feedback-externo-concept]]
