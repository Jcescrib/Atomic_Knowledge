---
type: aku
aku_class: claim
id: aku-subordinado-responsable-de-entender-porque-claim
statement: >
  Si no entiendes o no crees en las decisiones que bajan de tu liderazgo, es tu
  responsabilidad (Extreme Ownership) preguntar hasta entender cómo y por qué se
  toman; no conocer el porqué impide creer en la misión y, en una posición de
  liderazgo, es una receta para el fracaso e inaceptable.
origin: "Jocko Willink & Leif Babin, Extreme Ownership (2015)"
domain: [liderazgo, jocko, responsabilidad, creencia]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports:
    - aku-extreme-ownership-concept
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-preguntar-por-que-method
    - aku-believe-in-the-mission-concept
    - aku-creer-para-asumir-riesgos-y-convencer-claim
    - aku-creencia-del-lider-se-transmite-claim
    - aku-objetivos-alineados-organizacion-claim

sources:
  - raw/libros/jocko/extreme-ownership/extreme-ownership.md

created: 2026-06-05
updated: 2026-06-05
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-extreme-ownership-concept]]

**related** ↔ [[aku-preguntar-por-que-method]] · [[aku-believe-in-the-mission-concept]] · [[aku-creer-para-asumir-riesgos-y-convencer-claim]] · [[aku-creencia-del-lider-se-transmite-claim]] · [[aku-objetivos-alineados-organizacion-claim]]
