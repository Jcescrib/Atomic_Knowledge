---
type: aku
aku_class: concept
id: aku-factores-crecimiento-blitzscaling-concept
statement: >
  Los factores que impulsan el crecimiento en blitzscaling son cuatro: los
  network effects (cada usuario adicional aumenta el valor de la red para todos),
  la distribución (capacidad de alcanzar el mercado objetivo de forma sistemática,
  p. ej. PayPal sobre la red de eBay), el tamaño de mercado (actual y futuro; si
  el mercado es nuevo, medir el tamaño del problema) y los grandes márgenes de
  beneficio; implica que sin estos factores el crecimiento explosivo no es
  sostenible.
origin: "Reid Hoffman & Chris Yeh — «Blitzscaling»; The Power MBA — Módulo 4.5"
domain: [estrategia, crecimiento, blitzscaling, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related: [aku-network-effect-concept, aku-masa-critica-concept, aku-motor-crecimiento-concept]

sources:
  - raw/cursos/power-mba/4-5-blitzscaling/4-5-blitzscaling.md

created: 2026-06-03
updated: 2026-06-03
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-network-effect-concept]] · [[aku-masa-critica-concept]] · [[aku-motor-crecimiento-concept]]
