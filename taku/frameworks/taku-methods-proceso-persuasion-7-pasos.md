---
type: taku
taku_type: framework
id: taku-methods-proceso-persuasion-7-pasos
title: "METHODS: proceso de persuasión en 7 pasos (Kolenda)"
origin: "Nick Kolenda — Methods of Persuasion"
domain: [persuasion, influence, marketing-psychology]

when_to_use: "Para estructurar una secuencia de persuasión completa (venta, petición, campaña), planificando qué hacer antes, durante y después de la petición."
when_not_to_use: "Para una interacción puntual sin preparación previa posible, o cuando la relación exige transparencia total y cualquier técnica de influencia se percibiría como manipulación."

aku_links:
  justified_by:
    - id: aku-methods-proceso-de-persuasion-en-siete-pasos-concept
      link_validation: human-validated
      link_note: "El framework completo de 7 pasos."
    - id: aku-el-priming-influye-en-la-conducta-claim
      link_validation: human-validated
      link_note: "Mecanismo del paso 2 (elicitar actitudes)."
    - id: aku-methods-step1-moldear-la-percepcion-concept
      link_validation: human-validated
      link_note: "Fase ANTES: moldear percepción."
    - id: aku-methods-step3-activar-presion-social-concept
      link_validation: human-validated
      link_note: "Fase ANTES: presión social."
    - id: aku-methods-step5-optimizar-tu-mensaje-concept
      link_validation: human-validated
      link_note: "Fase DURANTE: optimizar mensaje."
    - id: aku-leyes-persuasion-cialdini-concept
      link_validation: human-validated
      link_note: "deducible: corrobora aku-methods-proceso-de-persuasion-en-siete-pasos-concept desde [power-mba] (cross-source)"
    - id: aku-system-1-vs-system-2-solo-somos-racionales-tras-ser-emocionales-concept
      link_validation: human-validated
      link_note: "deducible: corrobora aku-el-priming-influye-en-la-conducta-claim desde [james-clear] (cross-source)"
    - id: aku-perception-is-reality-value-claim
      link_validation: human-validated
      link_note: "deducible: corrobora aku-methods-step1-moldear-la-percepcion-concept desde [hormozi] (cross-source)"
    - id: aku-el-toque-ligero-en-el-antebrazo-aumenta-el-acuerdo-method
      link_validation: human-validated
      link_note: "deducible: corrobora aku-methods-step1-moldear-la-percepcion-concept desde [50-cent] (cross-source)"
    - id: aku-la-conducta-normal-de-la-tribu-vence-a-la-deseada-del-individuo-claim
      link_validation: human-validated
      link_note: "deducible: corrobora aku-methods-step3-activar-presion-social-concept desde [james-clear] (cross-source)"
    - id: aku-cialdini-prueba-social-concept
      link_validation: human-validated
      link_note: "deducible: corrobora aku-methods-step3-activar-presion-social-concept desde [power-mba] (cross-source)"
  constrained_by: []
  breaks_when: []
  illustrates: []
  challenges: []

content_validation:
  status: llm-authored
  reviewed_by: ""
  review_date: ""
  review_notes: ""

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

taku_relations:
  complementary: []
  alternative_to: []
  precedes: []
  follows: []

created: 2026-06-09
updated: 2026-06-09
status: draft
status_note: ""
---

## Summary

METHODS es un proceso secuencial de persuasión en siete pasos, agrupados en tres fases (antes, durante y después de la petición). El acrónimo nombra los siete pasos: **M**old, **E**licit, **T**rigger, **H**abituate, **O**ptimize, **D**rive, **S**ustain.

## Core Components

- **ANTES:** (1) Mold Their Perception · (2) Elicit Congruent Attitudes · (3) Trigger Social Pressure · (4) Habituate Your Message.
- **DURANTE:** (5) Optimize Your Message · (6) Drive Their Momentum.
- **DESPUÉS:** (7) Sustain Their Compliance.

## How to Apply

Recorrer los pasos en orden temporal. En la fase ANTES se prepara el terreno (percepción, actitudes vía priming, presión social, familiaridad por exposición); en la fase DURANTE se formula y entrega la petición optimizada generando momentum; en la fase DESPUÉS se consolida el compromiso para que perdure.

## Underlying Claims

Ver `aku_links.justified_by`. Cada paso es un concept-AKU del cluster `kolenda, methods-of-persuasion`; el priming respalda el paso 2.

## Strengths

- Cubre el ciclo completo de influencia (no solo el momento de pedir).
- Acrónimo memorable que ordena tácticas dispersas de la literatura de persuasión.

## Limitations and Criticisms

- El detalle de las tácticas concretas de cada paso vive en la narración del audiolibro, no en esta fuente.
- Roza la manipulación si se usa sin transparencia; exige criterio ético.
- Solapa con las leyes de Cialdini (presión social, consistencia).

## Variants and Extensions

- Combinar con las leyes de Cialdini y con el cluster `kolenda, copywriting` (paso 5, optimizar el mensaje).

## Relaciones

**justified_by** ← [[aku-methods-proceso-de-persuasion-en-siete-pasos-concept]] · [[aku-el-priming-influye-en-la-conducta-claim]] · [[aku-methods-step1-moldear-la-percepcion-concept]] · [[aku-methods-step3-activar-presion-social-concept]] · [[aku-methods-step5-optimizar-tu-mensaje-concept]] · [[aku-leyes-persuasion-cialdini-concept]] · [[aku-system-1-vs-system-2-solo-somos-racionales-tras-ser-emocionales-concept]] · [[aku-perception-is-reality-value-claim]] · [[aku-el-toque-ligero-en-el-antebrazo-aumenta-el-acuerdo-method]] · [[aku-la-conducta-normal-de-la-tribu-vence-a-la-deseada-del-individuo-claim]] · [[aku-cialdini-prueba-social-concept]]
