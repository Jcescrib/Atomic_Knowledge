---
type: aku
aku_class: concept
id: aku-identidad-de-marca-concept
statement: >
  La identidad de marca (brand identity) es el conjunto de signos distintivos de
  una marca —logo, imágenes, colores, tipografías, sonidos, etc.—; es la capa
  visible y tangible de la marca, distinta de su esencia (propósito, valores,
  posicionamiento); implica que centrarse en la identidad (el logo) sin tener
  clara la esencia es el error habitual de la mayoría de empresas.
origin: "The Power MBA — Módulo 4.4 «Branding»"
domain: [marketing, branding, marca, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-marca-concept
    - aku-esencia-mas-que-logo-claim
    - aku-plan-de-marca-concept
    - aku-el-poder-de-la-percepcion-concept
    - aku-font-psychology-las-fuentes-significan-por-congruencia-de-rasgos-concept
    - aku-las-marcas-desconocidas-deben-invertir-en-packaging-claim
    - aku-color-psychology-el-significado-del-color-depende-del-contexto-y-la-experiencia-concept
    - aku-primar-la-eleccion-con-diseno-visual-semantic-priming-claim
    - aku-marca-potente-beneficios-claim

sources:
  - raw/cursos/power-mba/4-4-branding/4-4-branding.md

created: 2026-06-03
updated: 2026-06-03
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-marca-concept]] · [[aku-esencia-mas-que-logo-claim]] · [[aku-plan-de-marca-concept]] · [[aku-el-poder-de-la-percepcion-concept]] · [[aku-font-psychology-las-fuentes-significan-por-congruencia-de-rasgos-concept]] · [[aku-las-marcas-desconocidas-deben-invertir-en-packaging-claim]] · [[aku-color-psychology-el-significado-del-color-depende-del-contexto-y-la-experiencia-concept]] · [[aku-primar-la-eleccion-con-diseno-visual-semantic-priming-claim]] · [[aku-marca-potente-beneficios-claim]]
