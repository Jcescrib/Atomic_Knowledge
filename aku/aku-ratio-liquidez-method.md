---
type: aku
aku_class: method
id: aku-ratio-liquidez-method
statement: >
  En el análisis de liquidez, el ratio de liquidez (o ratio de fondo de maniobra) se
  computa como el activo corriente dividido entre el pasivo corriente: Ratio de
  liquidez = Activo Corriente / Pasivo Corriente; un valor superior a 1 indica que el
  activo corriente cubre las deudas a corto plazo (fondo de maniobra positivo).
origin: "The Power MBA — Módulo 9.2 «Ratios de análisis financiero»"
domain: [finanzas, contabilidad, ratios, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: [aku-liquidez-concept, aku-fondo-de-maniobra-concept]
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related: []

sources:
  - raw/cursos/power-mba/09-02-01-ratios-analisis-financieros/09-02-01-ratios-analisis-financieros.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**supported_by** ← [[aku-liquidez-concept]] · [[aku-fondo-de-maniobra-concept]]
