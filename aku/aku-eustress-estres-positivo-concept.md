---
type: aku
aku_class: concept
id: aku-eustress-estres-positivo-concept
statement: >
  El eustress es la forma positiva del estrés: aporta motivación que nos mantiene
  avanzando hacia las metas (p. ej. el nerviosismo útil al estudiar un examen o
  preparar una entrevista importante); es la cantidad óptima de estrés para la
  persona media y el opuesto del distress, y su presencia es vital para la salud
  mental y el bienestar.
origin: "Rener Gracie & Paul Volponi — The 32 Principles (2023)"
domain: [jiu-jitsu, principios, jocko, tension-principle, psicologia]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-tension-principle-concept
    - aku-stress-bueno-y-malo-cortisol-claim
    - aku-miedo-al-fracaso-paraliza-y-neutraliza-claim
    - aku-responder-no-reaccionar-da-control-claim

sources:
  - raw/libros/jocko/32-principles/32-principles.md

created: 2026-06-05
updated: 2026-06-05
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-tension-principle-concept]] · [[aku-stress-bueno-y-malo-cortisol-claim]] · [[aku-miedo-al-fracaso-paraliza-y-neutraliza-claim]] · [[aku-responder-no-reaccionar-da-control-claim]]
