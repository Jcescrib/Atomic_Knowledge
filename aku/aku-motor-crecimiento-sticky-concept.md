---
type: aku
aku_class: concept
id: aku-motor-crecimiento-sticky-concept
statement: >
  El motor de crecimiento sticky (pegajoso) se basa en un lifetime largo del
  cliente —los clientes permanecen activos años— y por tanto en mantener el
  churn rate muy bajo; incluye modelos de suscripción y modelos con alta
  recurrencia natural de consumo; excluye modelos transaccionales puntuales
  donde no existe relación continuada con el cliente; implica foco operativo
  en retención (no en captación), CAC payback típicamente alto (modelo
  capital-intensivo que requiere financiación externa para escalar) y
  dificultad de calcular el CLTV de forma fiable en negocios jóvenes sin
  histórico representativo. Ejemplos: Netflix, Tulotero.
origin: "The Power MBA — Módulo 2.2.1 «Tipos de motores de crecimiento»"
domain: [growth, business-model, retention, subscription, capital-intensive]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-motor-crecimiento-concept
    - aku-motor-crecimiento-pago-concept
    - aku-motor-crecimiento-viral-concept

sources:
  - "raw/2-2-1-tiposmotorescrecimiento/2-2-1-tiposmotorescrecimiento.md"

created: 2026-06-03
updated: 2026-06-03
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-motor-crecimiento-concept]] · [[aku-motor-crecimiento-pago-concept]] · [[aku-motor-crecimiento-viral-concept]]
