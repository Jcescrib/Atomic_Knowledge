---
type: aku
aku_class: method
id: aku-rotacion-existencias-method
statement: >
  En el análisis de actividad, la rotación de existencias (o de inventario) se computa
  como el coste de las mercancías vendidas dividido entre las existencias medias:
  Rotación de existencias = Coste de mercancías vendidas / Existencias medias; mide
  cuántas veces se renueva el inventario en el periodo; una rotación alta indica
  gestión eficiente del stock.
origin: "The Power MBA — Módulo 9.2 «Ratios de análisis financiero»"
domain: [finanzas, contabilidad, ratios, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: [aku-coste-mercancias-vendidas-concept, aku-existencias-concept]
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related: []

sources:
  - raw/cursos/power-mba/09-02-01-ratios-analisis-financieros/09-02-01-ratios-analisis-financieros.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**supported_by** ← [[aku-coste-mercancias-vendidas-concept]] · [[aku-existencias-concept]]
