---
type: aku
aku_class: concept
id: aku-perfil-marketplace-implicaciones-concept
statement: >
  El perfil de implicaciones del modelo marketplace describe, dimensión a
  dimensión, las consecuencias de elegir un marketplace: riesgo alto, potencial de
  escalabilidad (compañías valiosas, exit), financiación intensiva en capital,
  punto muerto lejano, primeros pasos difíciles porque validar la propuesta de
  valor depende de alcanzar masa crítica, y conocimientos clave en estrategia de
  plataformas; implica que el modelo marketplace es escalable pero exige mucho
  capital y tiempo hasta la rentabilidad por su dependencia del efecto de red.
origin: "The Power MBA — Módulo 6.1 «Fit entre tu proyecto y tú»"
domain: [emprendimiento, modelo-de-negocio, marketplace, escalabilidad, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by:
    - aku-implicaciones-modelo-negocio-concept
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-marketplace-concept
    - aku-huevo-gallina-concept
    - aku-ingresos-punto-muerto-modelo-concept
    - aku-perfil-ecommerce-implicaciones-concept
    - aku-perfil-modelo-innovador-vs-menos-innovador-concept
    - aku-perfil-startup-vs-tradicional-concept

sources:
  - raw/cursos/power-mba/6-1-01-fit-entre-tu-proyecto-y-tu/6-1-01-fit-entre-tu-proyecto-y-tu.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**supported_by** ← [[aku-implicaciones-modelo-negocio-concept]]

**related** ↔ [[aku-marketplace-concept]] · [[aku-huevo-gallina-concept]] · [[aku-ingresos-punto-muerto-modelo-concept]] · [[aku-perfil-ecommerce-implicaciones-concept]] · [[aku-perfil-modelo-innovador-vs-menos-innovador-concept]] · [[aku-perfil-startup-vs-tradicional-concept]]
