---
type: aku
aku_class: concept
id: aku-effort-sacrifice-concept
statement: >
  El esfuerzo y sacrificio (effort & sacrifice) son los costes accesorios
  —tangibles e intangibles— que le cuesta al cliente «por el camino» lograr el
  resultado; reducirlos, o reducir su percepción, aumenta el valor; implica que
  los servicios «done for you» valen más que los «do it yourself» (menor
  esfuerzo y mayor probabilidad percibida de logro).
origin: "Alex Hormozi — $100M Offers"
domain: [value-proposition, hormozi]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports:
    - aku-value-equation-concept
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-coste-percibido-amplio-concept
    - aku-focus-bottom-value-equation-claim
    - aku-costes-ocultos-oferta-gratis-claim
    - aku-sacrificar-el-ego-coste-percibido-mayor-que-real-claim
    - aku-dream-outcome-concept
    - aku-perceived-likelihood-achievement-concept
    - aku-time-delay-value-concept
    - aku-perception-is-reality-value-claim
    - aku-status-driver-compra-claim

sources:
  - raw/libros/hormozi/100m-offers/100m-offers.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-value-equation-concept]]

**related** ↔ [[aku-coste-percibido-amplio-concept]] · [[aku-focus-bottom-value-equation-claim]] · [[aku-costes-ocultos-oferta-gratis-claim]] · [[aku-sacrificar-el-ego-coste-percibido-mayor-que-real-claim]] · [[aku-dream-outcome-concept]] · [[aku-perceived-likelihood-achievement-concept]] · [[aku-time-delay-value-concept]] · [[aku-perception-is-reality-value-claim]] · [[aku-status-driver-compra-claim]]
