---
type: aku
aku_class: concept
id: aku-lead-magnet-calculadora-concept
statement: >
  La calculadora es un tipo de lead magnet que ofrece una herramienta interactiva
  que computa un resultado personalizado para el usuario (p. ej. sueldo, fondo de
  emergencia) a cambio del dato del lead; implica aportar valor mediante un cálculo
  útil y específico al caso del usuario, y se distingue de los lead magnets de
  contenido estático por su carácter interactivo y de tooling.
origin: "The Power MBA — Módulo 5 «Estrategias de marketing digital»"
domain: [marketing-digital, captacion, lead-magnet, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by:
    - aku-lead-magnet-concept
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-lead-magnet-concurso-sorteo-concept
    - aku-lead-magnet-curso-gratuito-concept
    - aku-lead-magnet-descuento-cupon-concept
    - aku-lead-magnet-plantilla-template-concept
    - aku-lead-magnet-prueba-producto-concept
    - aku-lead-magnet-resumen-visita-concept
    - aku-lead-magnet-ubicaciones-claim

sources:
  - raw/cursos/power-mba/5-5-2-ejemplos-por-tipos-de-lead-magnets/5-5-2-ejemplos-por-tipos-de-lead-magnets.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**supported_by** ← [[aku-lead-magnet-concept]]

**related** ↔ [[aku-lead-magnet-concurso-sorteo-concept]] · [[aku-lead-magnet-curso-gratuito-concept]] · [[aku-lead-magnet-descuento-cupon-concept]] · [[aku-lead-magnet-plantilla-template-concept]] · [[aku-lead-magnet-prueba-producto-concept]] · [[aku-lead-magnet-resumen-visita-concept]] · [[aku-lead-magnet-ubicaciones-claim]]
