---
type: aku
aku_class: concept
id: aku-twitter-concept
statement: >
  Twitter es la red social de microblogging (~340 millones de usuarios activos),
  válida tanto para B2B como B2C y para todas las edades; incluye que mide la
  interacción por retweets y likes, tiene impacto mediano en tráfico web y se anuncia
  con Twitter Ads; implica que es ideal para cualquier negocio y especialmente para
  atención al cliente; su limitación principal es el límite de 280 caracteres.
origin: "The Power MBA — Módulo 12 «Social media» — Tabla comparativa RRSS"
domain: [marketing-digital, social-media, canales, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related: [aku-comparativa-redes-sociales-concept]

sources:
  - raw/cursos/power-mba/12-3-tabla-comparativa-rrss/12-3-tabla-comparativa-rrss.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-comparativa-redes-sociales-concept]]
