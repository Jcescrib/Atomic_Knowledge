---
type: aku
aku_class: concept
id: aku-eficiencia-apex-timing-control-macro-concept
statement: >
  El jiu-jitsu tiene una jerarquía de principios: la eficiencia es el principio
  apex (lograr el máximo resultado con el mínimo esfuerzo), mientras que el timing
  y el control son los macroprincipios bien conocidos; los 32 microprincipios se
  sitúan por debajo de estos y explican cómo se logra la eficiencia en cada técnica.
origin: "Rener Gracie & Paul Volponi — The 32 Principles (2023)"
domain: [jiu-jitsu, principios, jocko, eficiencia]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-32-principios-jiu-jitsu-concept
    - aku-maestro-nunca-deja-de-aprender-humildad-claim
    - aku-32-principles-diagnostic-method
    - aku-sinergia-principios-se-combinan-concept
    - aku-grandmaster-principle-concept
    - aku-principios-jiu-jitsu-forma-de-pensar-transferible-claim

sources:
  - raw/libros/jocko/32-principles/32-principles.md

created: 2026-06-05
updated: 2026-06-05
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-32-principios-jiu-jitsu-concept]] · [[aku-maestro-nunca-deja-de-aprender-humildad-claim]] · [[aku-32-principles-diagnostic-method]] · [[aku-sinergia-principios-se-combinan-concept]] · [[aku-grandmaster-principle-concept]] · [[aku-principios-jiu-jitsu-forma-de-pensar-transferible-claim]]
