---
type: aku
aku_class: concept
id: aku-no-clientes-concept
statement: >
  Los no-clientes son los segmentos del mercado que ningún competidor del
  sector está sirviendo actualmente y que la estrategia del océano azul
  propone como territorio prioritario para crear nuevos espacios; incluye
  tres niveles según Kim & Mauborgne (próximos no-clientes que están a
  punto de irse, no-clientes que rechazan la oferta del sector,
  no-clientes inexplorados de sectores muy distantes); excluye los
  clientes que ya están siendo disputados activamente en el océano rojo;
  implica que dirigirse a no-clientes obliga a re-formular la propuesta de
  valor —no a sub-segmentar más fino— y abre el mercado en lugar de
  partirlo.
origin: "The Power MBA — Módulo 2.6 «Cómo crear propuestas de valor disruptivas y buscar tu océano azul»"
domain: [strategy, blue-ocean, market-creation, targeting, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-oceano-azul-concept
    - aku-cliente-buscando-vs-no-concept

sources:
  - "raw/cursos/power-mba/2-6-oceano-azul-como-crear-propuestas-de-valor-disruptivas-y-buscar-para-tu-oc-eano-azul-conceptos-clave/2-6-oceano-azul-como-crear-propuestas-de-valor-disruptivas-y-buscar-para-tu-oc-eano-azul-conceptos-clave.md"

created: 2026-06-03
updated: 2026-06-03
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-oceano-azul-concept]] · [[aku-cliente-buscando-vs-no-concept]]
