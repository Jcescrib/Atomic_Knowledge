---
type: aku
aku_class: concept
id: aku-laws-of-combat-concept
statement: >
  Las Leyes del Combate (Laws of Combat) son los cuatro conceptos —Cover and
  Move (cubrir y avanzar), Simple, Prioritize and Execute (priorizar y ejecutar)
  y Decentralized Command (mando descentralizado)— que, comprendidos y aplicados
  conjuntamente, permiten a cualquier equipo rendir al máximo nivel y dominar;
  incluye que son interdependientes y mutuamente reforzantes; excluye tratarlos
  como tácticas aisladas.
origin: "Jocko Willink & Leif Babin, Extreme Ownership (2015)"
domain: [liderazgo, jocko, combate, equipos]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by:
    - aku-extreme-ownership-concept
    - aku-cover-and-move-concept
    - aku-simple-law-of-combat-concept
    - aku-prioritize-and-execute-concept
    - aku-decentralized-command-concept
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-principios-combate-aplican-a-negocio-claim
    - aku-simple-but-not-easy-concept
    - aku-relax-look-around-make-a-call-method
    - aku-estudiar-liderazgo-lente-de-liderazgo-claim

sources:
  - raw/libros/jocko/extreme-ownership/extreme-ownership.md

created: 2026-06-05
updated: 2026-06-05
status: active
status_note: ""
---

## Relaciones

**supported_by** ← [[aku-extreme-ownership-concept]] · [[aku-cover-and-move-concept]] · [[aku-simple-law-of-combat-concept]] · [[aku-prioritize-and-execute-concept]] · [[aku-decentralized-command-concept]]

**related** ↔ [[aku-principios-combate-aplican-a-negocio-claim]] · [[aku-simple-but-not-easy-concept]] · [[aku-relax-look-around-make-a-call-method]] · [[aku-estudiar-liderazgo-lente-de-liderazgo-claim]]
