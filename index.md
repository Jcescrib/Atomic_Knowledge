...
By AKU class: **483 concept**, **62 method**, **265 claim** · all `unvalidated` · `sourced` mayoría at `llm_confidence: 0.50`, ~22 `sourced` at `0.60` (post-dedup, incl. 8 métricas growth-engine tras merge fuente 5-2), 2 at `0.70` (lead-magnet 3 fuentes; cltv-cac-ratio 3 fuentes), 1 `sourced` at `0.20` (mas-valor-menos-coste), 1 `tacit` at `null` (Joan's free-bootstrap-plataforma claim).

Graph: **1 connected component** (893 nodos: 810 AKU + 83 TAKU), 0 bidirectional errors (810/810 simétrico, 0 body-drift, 0 wikilinks rotos, 0 huérfanos — verificado por `scripts/verify_graph.py`). **389 AKUs llevan el source-tag `hormozi`** (130 de la 1ª ingesta + 259 de la reingesta diferencial hiper-exhaustiva).
