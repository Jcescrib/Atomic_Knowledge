---
type: aku
aku_class: concept
id: aku-modelo-plataforma-concept
statement: >
  El modelo de plataforma es un modelo de negocio en el que la empresa crea
  una infraestructura dentro de la cual los usuarios se aportan valor entre
  ellos; incluye toda configuración con dos o más lados (segmentos de
  clientes) que interactúan a través de la plataforma; excluye los modelos
  lineales sin interacción usuario-usuario; implica la existencia de network
  effects, la necesidad de alcanzar masa crítica para empezar a generar
  valor, el reto del huevo y la gallina al lanzar, y subtipos diferenciables
  por la libertad/control que ejercen sobre las interacciones (marketplace,
  on-demand, content platform).
origin: "The Power MBA — Módulo 2.3 «Conceptos clave de las plataformas»"
domain: [business-model, platform, two-sided-market, network-effects]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-modelo-lineal-concept
    - aku-network-effect-concept
    - aku-masa-critica-concept
    - aku-circulo-virtuoso-concept
    - aku-circulo-vicioso-concept
    - aku-huevo-gallina-concept
    - aku-marketplace-concept
    - aku-on-demand-platform-concept
    - aku-content-platform-concept
    - aku-modelo-free-concept
    - aku-modelo-freemium-concept
    - aku-bmc-concept
    - aku-segmentos-de-clientes-concept

sources:
  - "raw/2-3-conceptos-clave-de-las-plataformas/2-3-conceptos-clave-de-las-plataformas.md"

created: 2026-06-03
updated: 2026-06-03
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-modelo-lineal-concept]] · [[aku-network-effect-concept]] · [[aku-masa-critica-concept]] · [[aku-circulo-virtuoso-concept]] · [[aku-circulo-vicioso-concept]] · [[aku-huevo-gallina-concept]] · [[aku-marketplace-concept]] · [[aku-on-demand-platform-concept]] · [[aku-content-platform-concept]] · [[aku-modelo-free-concept]] · [[aku-modelo-freemium-concept]] · [[aku-bmc-concept]] · [[aku-segmentos-de-clientes-concept]]
