---
type: aku
aku_class: concept
id: aku-calmado-no-robotico-concept
statement: >
  Dicotomía calmado-no robótico: el líder debe controlar sus emociones (quien
  pierde los nervios pierde el respeto), pero mostrar emoción es normal y
  necesario; un líder sin ninguna emoción parece un robot, y la gente no sigue a
  los robots.
origin: "Jocko Willink & Leif Babin, Extreme Ownership (2015)"
domain: [liderazgo, jocko, dicotomia]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports:
    - aku-dichotomy-of-leadership-concept
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-agresivo-significa-proactivo-no-iracundo-claim
    - aku-no-sobrerreaccionar-mantener-la-calma-claim

sources:
  - raw/libros/jocko/extreme-ownership/extreme-ownership.md

created: 2026-06-05
updated: 2026-06-05
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-dichotomy-of-leadership-concept]]

**related** ↔ [[aku-agresivo-significa-proactivo-no-iracundo-claim]] · [[aku-no-sobrerreaccionar-mantener-la-calma-claim]]
