---
type: aku
aku_class: concept
id: aku-medicion-resultados-influencers-concept
statement: >
  La medición de resultados de campañas con influencers asigna KPIs a cada etapa del
  funnel y define cómo medirlos: notoriedad/awareness (alcance, impresiones → analítica
  de la plataforma o informe del influencer), engagement (likes, comentarios, shares,
  menciones, hashtags, DMs → por contrato y analítica), visita (visitas a web/landing,
  % nuevos, tiempo por sesión, páginas vistas, bounce rate, leads → Google Analytics) y
  acción (ventas generadas, ratio de conversión → Google Analytics); implica exigir por
  contrato las estadísticas para poder calcular el retorno.
origin: "The Power MBA — Módulo 14 «Marketing de influencers» — Medición de resultados"
domain: [marketing-digital, influencer-marketing, analitica, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related: [aku-marketing-influencers-concept, aku-power-funnel-concept, aku-clausula-remuneracion-influencer-concept, aku-clausula-envio-resultados-influencer-concept]

sources:
  - raw/cursos/power-mba/14-06-medici-on-resultados-influencers-pptx/14-06-medici-on-resultados-influencers-pptx.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-marketing-influencers-concept]] · [[aku-power-funnel-concept]] · [[aku-clausula-remuneracion-influencer-concept]] · [[aku-clausula-envio-resultados-influencer-concept]]
