---
type: aku
aku_class: concept
id: aku-estructuras-organizacionales-concept
statement: >
  Hay cuatro estructuras organizativas comunes: funcional (agrupada por funciones
  —marketing, finanzas, operaciones—; sinergias de conocimiento pero pérdida de foco
  en el cliente), divisional (divisiones casi autónomas por geografía, producto,
  marca o segmento; orientadas al cliente pero con ineficiencias), matriz (híbrido de
  funciones y divisiones con doble reporte; más resultados pero más complejidad) y
  basada en proyectos (equipos temporales multifuncionales con objetivos acotados;
  ágil e innovadora pero inestable); implica elegir la estructura que sigue a la
  estrategia.
origin: "The Power MBA — Módulo 7.2 «Liderar a otros»"
domain: [leadership, organizacion, estructura, estrategia, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by:
    - aku-estructura-cultura-siguen-estrategia-claim
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-arquetipos-cultura-concept
    - aku-empleados-lead-getters-concept

sources:
  - raw/cursos/power-mba/7-2-1-recurso-liderar-a-otros/7-2-1-recurso-liderar-a-otros.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**constrained_by** ← [[aku-estructura-cultura-siguen-estrategia-claim]]
**related** ↔ [[aku-arquetipos-cultura-concept]] · [[aku-empleados-lead-getters-concept]]
