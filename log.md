...
## 2026-06-08 — ingest
Hustle Harder, Hustle Smarter — Cap. 8 «Learning from Your Ls».
- **+10 AKUs** (1 concept núcleo `aprender-de-tus-Ls-el-fracaso-como-herramienta`; 9 claim). Temas: éxito-sobre-fracaso-repetido (Honda 99%), la-respuesta>el-fracaso, abraza-el-fracaso/autopsia, fracaso-temprano→más-éxito-largo-plazo, admitir-error (conductor-perdido), yes-men-impiden-aprender, jefe-que-grita-silencia-feedback (Floyd/Bobby whipping-boy), si-eres-whipping-boy-cambia-energía, depender-de-otros-en-tus-negocios-es-peligroso (Don King).
- Puentes (b) propuestos para Joan: aprender-de-tus-Ls ↔ good-mindset/post-operational-debrief/extreme-ownership (Jocko), reflexión-y-revisión/never-miss-twice (Atomic Habits); admitir-error ↔ humildad-asumir-errores (Jocko) & `admitir-que-no-lo-sabes-todo` (Ch5); yes-men ↔ `cambiar-de-opinion`/`expande-tu-circulo` (Ch5).
- verify_graph: 1979/1979 simétrico, 0 errores.

## 2026-06-08 — ingest
Hustle Harder, Hustle Smarter — Cap. 9 «The Entitlement Trap» + framework TAKU (LIBRO COMPLETO).
- **+20 AKUs** (1 concept núcleo `la-trampa-del-entitlement-nadie-te-debe-nada`; 1 method `recuperarte-de-un-desastre`; 18 claim) + **1 framework TAKU** `taku-los-9-principios-hustle-harder-hustle-smarter` (draft, llm-authored, 10 justified_by llm-proposed: los 9 concept-AKU núcleo + concepto paraguas).
- Temas Ch9: aceptar-que-todo-depende-de-ti-libera, distancia-de-quien-te-arrastra (lifeguard), trabajo=felicidad, hacer-demasiado-crea-entitlement (Marquise), want-como-motor, ganártelo-multiplica-valor (la-montaña), culpar-a-otros (Michael), did-vs-would-have, depresión-es-lujo, responsabilidad-total-aunque-no-sea-tu-culpa (Freud), recuperación-de-desastre, entitlement-cultural/learned-helplessness, depósitos-vs-retiros, haz-depósitos/puntualidad, aumento-por-contribución-no-antigüedad, los-que-dan/conscious-capitalism, pobreza-real-cura-entitlement.
...
## 2026-06-09 — ingest: kolenda 11/16 — negotiation-tactics (máximo-exhaustivo)
- **+31 AKUs**: CORE concept (`negotiation-tactics-influencia-psicologica-por-fase`) + **28 tácticas** (1 por táctica, por fase: 8 BEFORE, 18 DURING, 2 AFTER) + 2 conceptos transversales (`batna`, `mentalidad-de-pastel-fijo-fixed-pie`). Generador `_kolenda_negotiation.py`.
- **Wiring (46 aristas)**: 30 `supports`→CORE; BATNA `supports`→{mejorar-batnas, mencionar-batnas}; abordar-todos-los-terminos `constrains`→fixed-pie. **Puentes cross-corpus**: hacer-primera-oferta `supported_by`→anchor-principle + related→exponer-numero-alto (anclaje, Pricing/Hormozi); pedir-rango-preciso→ser-preciso-con-precios + numero-en-la-cabeza-perdida (Pricing/50 Cent); llevar-café-pasteles + contingencia `supported_by`→cialdini-reciprocidad; contingencia→pedir-de-nuevo-con-bono (50 Cent); enmarcar-venta-como-formación→autonomía-but-you-are-free (reactancia, Copywriting); primar-capacidad-ejecutar `supported_by`→simulation-fluency (Choice); elegir-hora-temprana→opción-primera-o-última (serial position, Choice); contraofertar + reunir-benchmark→nunca-fijarse-en-número + claridad-de-objetivo (50 Cent).
- **Fase 3 figuras**: 10 ilustraciones de táctica embebidas (`![[hash.jpg]]` bajo `## Figura`); las 21 imágenes de portada/ToC/divisores de sección omitidas por decorativas.
- verify_graph: 2276/2276 simétrico, **0 errores**. 1 componente, 0 huérfanos. **Kolenda 11/16, vault 2245→2276.**

## 2026-06-09 — ingest: kolenda 12/16 — naming-a-product (máximo-exhaustivo)
- **+58 AKUs**: CORE concept + **Step 1** (matriz wording×relevance + 4 tipos {descriptivo, asociativo, deviant, neologístico} + 8 dimensiones de selección {persuasivo, memorable, distintivo, relevante, emocional, escalable, protegible, SEO}) + **Step 2** (sound-symbolism + arbitrariedad-necesaria + 5 fuentes {frequency-code, perceptual/kinesthetic-fluency, facial-feedback, blending} + 3 categorías fonémicas {anterior/posterior, sonora/sorda, fricativa/oclusiva} + Grupo 1/Grupo 2 + 7 significados fonémicos {tamaño, forma, velocidad, luz, belleza, plazo, género} + fonestemas) + **Step 3** (4 métodos de construcción + 6 claims {plosiva-recall, inward-articulation, género-final, acento-N/V, beneficio-positivo, sustantivos-concretos}) + **Step 4** (8 filtros {longitud, pronunciación-fácil, disfluencia-avanzada, repetición-fonética, escritura, abreviatura, traducción, disponibilidad}) + **Step 5** (7 reglas alfanuméricas {productos-técnicos, números-implican-atributos, mayor=avanzado+anclaje, simple, aliterado, redondo, compuesto}). Generador `_kolenda_naming.py`.
- **+2 TAKUs** (draft, llm-authored): `taku-metodologia-naming-5-pasos` (framework, 5 justified_by) + `taku-kit-tecnicas-generativas-de-nombres` (tool, 13 técnicas: blend/prefix/suffix/removal/replacement/homophone/onomatopoeia/translation/acronym/alliteration/rhyme/founder/geography; 4 justified_by); complementary entre sí.
- **Wiring (75 aristas)**: supports→CORE + internas (grupos/fuentes→métodos) + **puentes cross-corpus**: sound-symbolism + perceptual-fluency→font-psychology (forma→significado); fonemas-angulares→fuentes-angulares (Font); género-fonético→texturas-rugosas-masculinas (Packaging); plosiva-inicio→left-digit-encoding, número-mayor→anclaje, números-redondos→precios-redondos, números-aliterados→aliteración-precios (Pricing).
- **Fase 3 figuras**: 16 figuras informacionales embebidas (scatterplots de la matriz por dimensión, gráficas Grupo 1/Grupo 2 de tamaño/forma/plazo, espectro vocálico, blending -ash, articulación inward).
- verify_graph: 2334/2334 simétrico, **0 errores**. 1 componente, 0 huérfanos. **Kolenda 12/16, vault 2276→2334.**

## 2026-06-09 — ingest: kolenda 13/16 — user-experience (máximo-exhaustivo)
- **+38 AKUs**: CORE + **5 guideline-concepts** (FOCUS, UNDERSTANDING, EFFORT, ERRORS, COMPATIBILITY) + **28 principios-táctica** (claim, p.ej. punto-de-entrada, flujo-visual, agrupar-por-proximidad-gestalt, data-ink, affordance, feedback, términos-relativos, scannability, choice-overload, minimizar-espera, recognition-over-recall, Fitts, prevenir-restringiendo, poka-yoke, undo, extender-clicables, accesibilidad…) + **4 sub-claims con mecanismo propio** (endowed-progress, default-recomendado, colores-fríos-espera, evitar-«tú»-en-errores). Generador `_kolenda_ux.py`.
- **+1 framework TAKU** (draft, llm-authored): `taku-checklist-usabilidad-ux-5-guidelines` — checklist operativo completo con los **~90 sub-tips imperativos** preservados en el body (qué hacer), 5 justified_by (las guidelines). Decisión de modelado: principios→AKU, sub-tips imperativos puros→TAKU (respeta límite AKU/TAKU sin omitir nada).
...
## 2026-06-09 — ingest: kolenda 15/16 + 16/16 — the-tangled-mind + methods-of-persuasion
- **the-tangled-mind (bonus-pdf)** — companion de figuras sin prosa (como IRTB). **+1 CORE concept**: `aku-the-tangled-mind-la-cognicion-es-corporal-embodied-concept` (embodied cognition / metáfora sensorial: el cuerpo confunde lo físico con lo abstracto — calor=calidez, peso=importancia, dureza=rigidez); figura embebida; `related`→ escala-de-decisión (Choice), silla-blanda (Negotiation), kinesthetic-fluency + inward (Naming). Es el mecanismo citado en otros libros Kolenda. NO se fabrican AKUs de figuras sin caption.
