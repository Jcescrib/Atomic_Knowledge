---
type: aku
aku_class: concept
id: aku-masa-critica-concept
statement: >
  La masa crítica es el umbral mínimo de usuarios que una plataforma necesita
  alcanzar para empezar a generar valor sostenido y disparar el círculo
  virtuoso; incluye el umbral específico de cada lado de una plataforma
  multi-side (e.g., en un marketplace, suficientes vendedores para generar
  valor a compradores y viceversa); excluye los modelos lineales (que generan
  valor desde el primer cliente); implica que antes de alcanzarla la
  plataforma opera en círculo vicioso, y por tanto que el reto operativo
  central del lanzamiento es romper esa fase no-rentable hasta que el
  network effect tome el relevo.
origin: "The Power MBA — Módulo 2.3 «Conceptos clave de las plataformas»"
domain: [business-model, platform, network-effects, growth, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-modelo-plataforma-concept
    - aku-circulo-virtuoso-concept
    - aku-huevo-gallina-concept
    - aku-free-bootstrap-plataforma-claim
    - aku-barreras-de-entrada-concept
    - aku-factores-crecimiento-blitzscaling-concept
    - aku-blitzscaling-cuando-oportunidad-enorme-claim

sources:
  - "raw/cursos/power-mba/2-3-conceptos-clave-de-las-plataformas/2-3-conceptos-clave-de-las-plataformas.md"

created: 2026-06-03
updated: 2026-06-03
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-modelo-plataforma-concept]] · [[aku-circulo-virtuoso-concept]] · [[aku-huevo-gallina-concept]] · [[aku-free-bootstrap-plataforma-claim]] · [[aku-barreras-de-entrada-concept]] · [[aku-factores-crecimiento-blitzscaling-concept]] · [[aku-blitzscaling-cuando-oportunidad-enorme-claim]]
