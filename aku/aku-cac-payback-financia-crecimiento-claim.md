---
type: aku
aku_class: claim
id: aku-cac-payback-financia-crecimiento-claim
statement: >
  En un motor de crecimiento de pago, cuanto antes se recupera el CAC (menor
  CAC payback), menos financiación externa adicional se necesita para crecer,
  porque las propias operaciones financian el crecimiento: el margen recuperado
  se reinvierte en captación. Implica que un CAC payback corto convierte al
  negocio en auto-financiable y que un payback largo, si se quiere crecer
  agresivamente, obliga a recursos adicionales.
origin: "The Power MBA — Módulo 2.2.1 «Tipos de motores de crecimiento»"
domain: [growth, unit-economics, cash-flow, business-model, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by:
    - aku-client-financed-acquisition-concept
    - aku-money-model-rompe-cash-bottleneck
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-motor-crecimiento-pago-concept
    - aku-cac-payback-concept
    - aku-money-model-concept
    - aku-coeficiente-viralidad-concept
    - aku-motor-crecimiento-concept
    - aku-motor-crecimiento-sticky-concept
    - aku-motor-crecimiento-viral-concept
    - aku-viral-bajo-cltv-compatible-claim

sources:
  - "raw/cursos/power-mba/2-2-1-tiposmotorescrecimiento/2-2-1-tiposmotorescrecimiento.md"

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-motor-crecimiento-pago-concept]] · [[aku-cac-payback-concept]] · [[aku-money-model-concept]] · [[aku-coeficiente-viralidad-concept]] · [[aku-motor-crecimiento-concept]] · [[aku-motor-crecimiento-sticky-concept]] · [[aku-motor-crecimiento-viral-concept]] · [[aku-viral-bajo-cltv-compatible-claim]]

**supported_by** ← [[aku-client-financed-acquisition-concept]] · [[aku-money-model-rompe-cash-bottleneck]]
