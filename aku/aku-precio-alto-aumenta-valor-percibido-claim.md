---
type: aku
aku_class: claim
id: aku-precio-alto-aumenta-valor-percibido-claim
statement: >
  Subir el precio aumenta directamente el valor percibido de un producto sin
  cambiar nada más (estudio del vino: el mismo vino se valora como mejor cuanto
  más caro se presenta); además, fijar un precio muy por encima del mercado crea
  «allure» y posiciona el producto en una categoría de uno con beneficios de
  monopolio.
origin: "Alex Hormozi — $100M Offers"
domain: [pricing, psicologia, comportamiento-compra, hormozi]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-virtuous-cycle-of-price-claim
    - aku-categoria-de-uno-concept
    - aku-emocion-prevalece-razon-claim
    - aku-cobrar-premium-decision-moral-claim
    - aku-reducir-costes-no-precio-claim

sources:
  - raw/libros/hormozi/100m-offers/100m-offers.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-virtuous-cycle-of-price-claim]] · [[aku-categoria-de-uno-concept]] · [[aku-emocion-prevalece-razon-claim]] · [[aku-cobrar-premium-decision-moral-claim]] · [[aku-reducir-costes-no-precio-claim]]
