---
type: aku
aku_class: claim
id: aku-entrenamiento-desde-abajo-no-desde-arriba-claim
statement: >
  Los mejores programas de entrenamiento no se orquestan desde arriba sino que
  se impulsan desde abajo —los líderes de primera línea más cercanos a la acción
  y las lecciones aprendidas—; hay que tomar Extreme Ownership del entrenamiento
  y usar a los miembros más capaces para dirigirlo; «no hay presupuesto» y «no
  hay tiempo» para entrenar no son excusas válidas.
origin: "Jocko Willink & Leif Babin, The Dichotomy of Leadership (2018)"
domain: [liderazgo, jocko, entrenamiento, iniciativa]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports:
    - aku-train-hard-but-train-smart-concept
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-extreme-ownership-concept
    - aku-analisis-constante-medir-efectividad-claim
    - aku-train-how-you-fight-claim
    - aku-entrenamiento-realismo-fundamentos-repeticion-method

sources:
  - raw/libros/jocko/dichotomy-of-leadership/dichotomy-of-leadership.md

created: 2026-06-05
updated: 2026-06-05
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-train-hard-but-train-smart-concept]]

**related** ↔ [[aku-extreme-ownership-concept]] · [[aku-analisis-constante-medir-efectividad-claim]] · [[aku-train-how-you-fight-claim]] · [[aku-entrenamiento-realismo-fundamentos-repeticion-method]]
