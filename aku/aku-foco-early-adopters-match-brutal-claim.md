---
type: aku
aku_class: claim
id: aku-foco-early-adopters-match-brutal-claim
statement: >
  De todos los nichos o customer personas posibles, lo mejor es centrarse de
  forma estricta en aquellos con los que la propuesta de valor encaja a la
  perfección —donde el «match» es brutal—, en lugar de intentar convencer a
  muchos segmentos a la vez; implica que diluir el foco entre varios segmentos
  produce una propuesta y unos mensajes que no son potentes realmente para
  nadie, mientras que el foco extremo en el cliente ideal genera la propuesta
  más potente.
origin: "The Power MBA — Módulo 2.4 «Segmentación y targeting»"
domain: [targeting, early-adopters, product-market-fit, marketing, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-early-adopter-concept
    - aku-nicho-strategy-concept
    - aku-comparativa-mass-market-vs-nicho-concept
    - aku-mass-market-strategy-concept
    - aku-segmentado-strategy-concept
    - aku-nicho-recomendado-lanzamiento-claim
    - aku-importancia-segmentacion-claim

sources:
  - "raw/cursos/power-mba/2-4-segmentaci-on-y-targeting-conceptos-clave-clientes-y-mercado-objetivo/2-4-segmentaci-on-y-targeting-conceptos-clave-clientes-y-mercado-objetivo.md"

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-early-adopter-concept]] · [[aku-nicho-strategy-concept]] · [[aku-comparativa-mass-market-vs-nicho-concept]] · [[aku-mass-market-strategy-concept]] · [[aku-segmentado-strategy-concept]] · [[aku-nicho-recomendado-lanzamiento-claim]] · [[aku-importancia-segmentacion-claim]]
