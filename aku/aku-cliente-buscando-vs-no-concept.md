---
type: aku
aku_class: concept
id: aku-cliente-buscando-vs-no-concept
statement: >
  El mercado de un producto se divide estructuralmente entre clientes que
  están buscando una solución activamente (~20% del total) y clientes que no
  están buscando (~80%); incluye en el primer grupo a quienes ya tienen
  claro lo que quieren y están comparando alternativas, y en el segundo a
  quienes no sienten el problema o sí lo sienten pero no han iniciado la
  búsqueda; excluye una mirada uniforme del mercado total; implica que
  competir solo por los que están buscando es entrar en un océano rojo de
  alta competencia y baja probabilidad de cierre, mientras que dirigirse a
  los que no están buscando ofrece mercado mucho mayor a cambio del reto de
  encontrarlos, generarles la necesidad y posicionarte como referencia.
origin: "The Power MBA — Módulo 2.4 «Segmentación y targeting»"
domain: [targeting, market-structure, demand, blue-ocean, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-segmentos-de-clientes-concept
    - aku-no-clientes-concept
    - aku-oceano-rojo-concept
    - aku-oceano-azul-concept
    - aku-lead-magnet-concept
    - aku-starving-crowd-concept
    - aku-dirigirse-a-no-buscadores-mayor-conversion-claim

sources:
  - "raw/cursos/power-mba/2-4-segmentaci-on-y-targeting-conceptos-clave-clientes-y-mercado-objetivo/2-4-segmentaci-on-y-targeting-conceptos-clave-clientes-y-mercado-objetivo.md"

created: 2026-06-03
updated: 2026-06-03
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-segmentos-de-clientes-concept]] · [[aku-no-clientes-concept]] · [[aku-oceano-rojo-concept]] · [[aku-oceano-azul-concept]] · [[aku-lead-magnet-concept]] · [[aku-starving-crowd-concept]] · [[aku-dirigirse-a-no-buscadores-mayor-conversion-claim]]
