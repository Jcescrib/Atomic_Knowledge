---
type: taku
taku_type: heuristic
id: taku-habitos-de-felicidad-naval
title: "Hábitos de felicidad de Naval"
origin: "Naval Ravikant — The Almanack of Naval Ravikant (ed. Eric Jorgenson, 2020)"
domain: [naval, felicidad, habitos]

when_to_use: "Para subir tu baseline de felicidad de forma deliberada mediante hábitos diarios entrenables."
when_not_to_use: "Como recetas universales garantizadas: la felicidad es trial-and-error y muy personal; lo que funciona para uno puede no servir a otro."

aku_links:
  justified_by:
    - id: aku-la-felicidad-es-una-habilidad-que-se-aprende-concept
      link_validation: human-validated
      link_note: ""
    - id: aku-mejora-metodica-de-tu-baseline-de-felicidad-method
      link_validation: human-validated
      link_note: ""
    - id: aku-felicidad-requiere-presencia-claim
      link_validation: human-validated
      link_note: ""
    - id: aku-toda-actividad-de-pantalla-resta-felicidad-claim
      link_validation: human-validated
      link_note: ""
    - id: aku-minimalismo-digital-concept
      link_validation: human-validated
      link_note: "deducible: corrobora aku-mejora-metodica-de-tu-baseline-de-felicidad-method desde [power-mba] (cross-source)"
    - id: aku-la-meditacion-con-mantra-baja-el-volumen-de-los-pensamientos-method
      link_validation: human-validated
      link_note: "deducible: corrobora aku-mejora-metodica-de-tu-baseline-de-felicidad-method desde [50-cent] (cross-source)"
    - id: aku-redefinir-agotamiento-eleva-baseline-claim
      link_validation: human-validated
      link_note: "deducible: corrobora aku-mejora-metodica-de-tu-baseline-de-felicidad-method desde [jocko] (cross-source)"
    - id: aku-el-entorno-es-la-mano-invisible-que-moldea-la-conducta-claim
      link_validation: human-validated
      link_note: "deducible: corrobora aku-toda-actividad-de-pantalla-resta-felicidad-claim desde [james-clear] (cross-source)"
  constrained_by: []
  breaks_when: []
  illustrates: []
  challenges: []

content_validation:
  status: llm-authored
  reviewed_by: ""
  review_date: ""
  review_notes: ""

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

taku_relations:
  complementary: []
  alternative_to: []
  precedes: []
  follows: []

created: 2026-06-05
updated: 2026-06-05
status: draft
status_note: ""
---

## The Rule

Trata la felicidad como una habilidad entrenable y aplica hábitos diarios, probando por trial-and-error qué funciona para ti:

- **Meditación insight**: observar cómo funciona tu mente.
- **Reinterpretación positiva**: al pillarte juzgando, pregúntate «¿cuál es la lectura positiva de esto?».
- **Cuestiona el deseo**: «¿esto es tan importante como para ser infeliz si no sale a mi modo?».
- **Cuerpo**: sol en la piel, sonríe mirando arriba, entrena a diario (paz del cuerpo → paz de la mente).
- **Quita estimulantes/dopamina barata**: deja cafeína, alcohol, azúcar; minimiza redes y videojuegos.
- **Minimiza tres apps**: teléfono, calendario y despertador.
- **Menos secretos**, menos pantallas, más juegos de suma positiva.
- **Serotonina sin drogas**: sol, ejercicio, pensamiento positivo, triptófano.
- **Declara que eres feliz** a tus amigos (sesgo de consistencia).
- **Reset en bajón**: meditación, música y ejercicio; luego elige un nuevo rumbo para el día.

## What It Replaces

Sustituye la búsqueda de felicidad en circunstancias externas y la dopamina barata de pantallas por hábitos internos sostenibles.

## When It Holds

Cuando los practicas religiosamente hasta volverlos segunda naturaleza y priorizas la felicidad por encima de otras metas.

## When It Fails

Si los aplicas como dogma sin experimentar cuál te sirve, o si esperas resultados sin constancia; ningún hábito sustituye a la aceptación.

## Why It Works

La paz y la felicidad son habilidades condicionables; sustituir hábitos malos irreflexivos por buenos, y el entorno (cinco chimpancés) por gente feliz, eleva el baseline.

## Calibration

Cambia un hábito cada vez con el método de cambio de hábitos (elige uno, visualiza, trackea, díselo a amigos) y mide tu métrica personal: cuánto del día haces por obligación vs. por interés.

## Relaciones

**justified_by** ← [[aku-la-felicidad-es-una-habilidad-que-se-aprende-concept]] · [[aku-mejora-metodica-de-tu-baseline-de-felicidad-method]] · [[aku-felicidad-requiere-presencia-claim]] · [[aku-toda-actividad-de-pantalla-resta-felicidad-claim]] · [[aku-minimalismo-digital-concept]] · [[aku-la-meditacion-con-mantra-baja-el-volumen-de-los-pensamientos-method]] · [[aku-redefinir-agotamiento-eleva-baseline-claim]] · [[aku-el-entorno-es-la-mano-invisible-que-moldea-la-conducta-claim]]
