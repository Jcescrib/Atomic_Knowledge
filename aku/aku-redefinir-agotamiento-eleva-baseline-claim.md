---
type: aku
aku_class: claim
id: aku-redefinir-agotamiento-eleva-baseline-claim
statement: >
  Desafiar y redefinir lo que significa estar agotado eleva el baseline de
  rendimiento: entrenar en condiciones más duras (arena profunda, altitud) o por
  intervalos progresivos «engaña» al cuerpo y la mente para soportar más, de modo
  que el umbral de agotamiento se desplaza y lo que antes parecía el límite pasa a
  ser sostenible.
origin: "Rener Gracie & Paul Volponi — The 32 Principles (2023)"
domain: [jiu-jitsu, principios, jocko, false-surrender-principle, entrenamiento, resiliencia]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-false-surrender-principle-concept
    - aku-depletion-principle-concept
    - aku-mejora-metodica-de-tu-baseline-de-felicidad-method
    - aku-asimetria-burn-rate-energia-claim
    - aku-load-management-pacing-recursos-finitos-concept
    - aku-detener-progreso-agota-al-rival-claim

sources:
  - raw/libros/jocko/32-principles/32-principles.md

created: 2026-06-05
updated: 2026-06-05
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-false-surrender-principle-concept]] · [[aku-depletion-principle-concept]] · [[aku-mejora-metodica-de-tu-baseline-de-felicidad-method]] · [[aku-asimetria-burn-rate-energia-claim]] · [[aku-load-management-pacing-recursos-finitos-concept]] · [[aku-detener-progreso-agota-al-rival-claim]]
