---
type: aku
aku_class: concept
id: aku-implicit-egotism-concept
statement: >
  El efecto implicit-egotism describe que nos sentimos atraídos por las cosas y
  personas que más se nos parecen; implica que nombrar la oferta llamando
  explícitamente al avatar (cuanto más local y específico mejor: Towson en vez
  de Baltimore) eleva la conversión porque el prospecto se reconoce en el
  nombre.
origin: "Alex Hormozi — $100M Offers"
domain: [oferta, hormozi]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports:
    - aku-elegir-modelos-que-se-parezcan-a-cada-segmento-claim
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-magic-naming-concept
    - aku-ajustar-los-numerales-al-nombre-o-cumpleanos-egotismo-implicito-claim
    - aku-naming-metodologia-cinco-pasos-concept
    - aku-riches-in-niches-pricing-claim
    - aku-dolor-es-el-pitch-claim

sources:
  - raw/libros/hormozi/100m-offers/100m-offers.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-elegir-modelos-que-se-parezcan-a-cada-segmento-claim]]
**related** ↔ [[aku-magic-naming-concept]] · [[aku-ajustar-los-numerales-al-nombre-o-cumpleanos-egotismo-implicito-claim]] · [[aku-naming-metodologia-cinco-pasos-concept]] · [[aku-riches-in-niches-pricing-claim]] · [[aku-dolor-es-el-pitch-claim]]
