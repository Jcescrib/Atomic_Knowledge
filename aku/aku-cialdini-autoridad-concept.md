---
type: aku
aku_class: concept
id: aku-cialdini-autoridad-concept
statement: >
  El principio de autoridad (Cialdini) sostiene que preferimos —y nos genera
  confort— comprar a personas o marcas que son una referencia o autoridad en su
  campo; en marketing se aplica destacando éxitos, logros, clientes, premios y
  credenciales que posicionen a la marca como experta.
origin: "Robert Cialdini — «Influence»; The Power MBA — Módulo 5.1 «Power Selling»"
domain: [marketing, persuasion, psicologia]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: [aku-leyes-persuasion-cialdini-concept]
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related: [aku-marca-potente-beneficios-claim]

sources:
  - raw/5-1-clave-powerselling/5-1-clave-powerselling.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-leyes-persuasion-cialdini-concept]]
**related** ↔ [[aku-marca-potente-beneficios-claim]]
