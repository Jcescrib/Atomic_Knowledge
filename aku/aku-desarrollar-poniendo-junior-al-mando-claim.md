---
type: aku
aku_class: claim
id: aku-desarrollar-poniendo-junior-al-mando-claim
statement: >
  Se desarrolla a jugadores de alto nivel poniendo a los junior al mando de
  operaciones de entrenamiento: comprenden cómo su trabajo encaja en la misión
  estratégica y se vuelven mejores en su puesto y como futuros líderes (caso
  Freddie: «bajo riesgo, estos saben liderar»).
origin: "Jocko Willink, Leadership Strategy and Tactics: Field Manual (2020)"
domain: [liderazgo, jocko, desarrollo, descentralizacion]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports:
    - aku-liderazgo-como-herramienta-para-ensenar-concept
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-liderazgo-en-todos-los-niveles-claim
    - aku-decentralizar-proceso-planificacion-claim
    - aku-ensenar-humildad-con-mision-dificil-method
    - aku-arreglar-mala-actitud-con-responsabilidad-que-importa-claim

sources:
  - raw/libros/jocko/leadership-strategy-and-tactics/leadership-strategy-and-tactics.md

created: 2026-06-05
updated: 2026-06-05
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-liderazgo-como-herramienta-para-ensenar-concept]]

**related** ↔ [[aku-liderazgo-en-todos-los-niveles-claim]] · [[aku-decentralizar-proceso-planificacion-claim]] · [[aku-ensenar-humildad-con-mision-dificil-method]] · [[aku-arreglar-mala-actitud-con-responsabilidad-que-importa-claim]]
