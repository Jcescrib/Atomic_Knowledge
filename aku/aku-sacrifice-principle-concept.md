---
type: aku
aku_class: concept
id: aku-sacrifice-principle-concept
statement: >
  El Sacrifice Principle consiste en entregar algo de valor real o percibido para
  ganar una ventaja táctica de otra forma; normalmente se activa cuando los demás
  métodos de avance se han agotado, e idealmente la ofrenda es un señuelo: al
  agarrar el sacrificio aparente, el oponente abre una ventana de oportunidad para
  revertir la situación.
origin: "Rener Gracie & Paul Volponi — The 32 Principles (2023)"
domain: [jiu-jitsu, principios, jocko, sacrifice-principle, estrategia]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports:
    - aku-sacrificio-real-vs-decoy-concept
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-isolation-principle-concept
    - aku-false-surrender-principle-concept
    - aku-momentum-principle-concept
    - aku-sacrificar-el-ego-coste-percibido-mayor-que-real-claim

sources:
  - raw/libros/jocko/32-principles/32-principles.md

created: 2026-06-05
updated: 2026-06-05
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-sacrificio-real-vs-decoy-concept]]

**related** ↔ [[aku-isolation-principle-concept]] · [[aku-false-surrender-principle-concept]] · [[aku-momentum-principle-concept]] · [[aku-sacrificar-el-ego-coste-percibido-mayor-que-real-claim]]
