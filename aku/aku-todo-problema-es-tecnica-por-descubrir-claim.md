---
type: aku
aku_class: claim
id: aku-todo-problema-es-tecnica-por-descubrir-claim
statement: >
  Todo problema es una técnica esperando a ser descubierta: para cada peor
  escenario existe una solución, y creer que esa solución existe es prerrequisito
  para hallarla; quien asume que el problema no tiene salida deja de buscar y por
  eso no la encuentra.
origin: "Rener Gracie & Paul Volponi — The 32 Principles (2023)"
domain: [jiu-jitsu, principios, jocko, mentalidad, resolucion-de-problemas]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-32-principios-jiu-jitsu-concept
    - aku-32-principles-diagnostic-method
    - aku-la-vida-pasa-para-ti-no-a-ti-claim
    - aku-river-principle-concept
    - aku-fijarse-en-obstaculo-lo-agranda-claim

sources:
  - raw/libros/jocko/32-principles/32-principles.md

created: 2026-06-05
updated: 2026-06-05
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-32-principios-jiu-jitsu-concept]] · [[aku-32-principles-diagnostic-method]] · [[aku-la-vida-pasa-para-ti-no-a-ti-claim]] · [[aku-river-principle-concept]] · [[aku-fijarse-en-obstaculo-lo-agranda-claim]]
