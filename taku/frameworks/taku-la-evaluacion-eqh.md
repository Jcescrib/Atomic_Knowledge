---
type: taku
taku_type: framework
id: taku-la-evaluacion-eqh
title: "La Evaluación del Eminently Qualified Human"
origin: "Jocko Willink, Dave Berke & Sarah Armstrong — The Code. The Evaluation. The Protocols. (2020)"
domain: [desarrollo-personal, jocko, the-evaluation, autoevaluacion]

when_to_use: "Para autoevaluarte a diario contra un estándar exigente en las seis áreas críticas de la vida y dirigir tu mejora."
when_not_to_use: "Para compararte con otras personas o como sello de estatus alcanzado: la Evaluación es you vs you y mide un camino que no termina."

aku_links:
  justified_by:
    - id: aku-the-evaluation-concept
      link_validation: human-validated
      link_note: ""
    - id: aku-evaluation-scoring-0-5-method
      link_validation: human-validated
      link_note: ""
    - id: aku-the-code-concept
      link_validation: human-validated
      link_note: ""
    - id: aku-eminently-qualified-human-concept
      link_validation: human-validated
      link_note: ""
    - id: aku-autoevaluacion-cuerpo-mente-alma-concept
      link_validation: human-validated
      link_note: "deducible: corrobora aku-the-evaluation-concept desde [power-mba] (cross-source)"
    - id: aku-sistemas-vs-metas-en-atomic-habits-concept
      link_validation: human-validated
      link_note: "deducible: corrobora aku-the-evaluation-concept desde [james-clear] (cross-source)"
    - id: aku-claridad-objetivos-fortalezas-exito-claim
      link_validation: human-validated
      link_note: "deducible: corrobora aku-the-evaluation-concept desde [power-mba] (cross-source)"
    - id: aku-tres-capas-liderazgo-concept
      link_validation: human-validated
      link_note: "deducible: corrobora aku-the-code-concept desde [power-mba] (cross-source)"
    - id: aku-objetivos-personales-concept
      link_validation: human-validated
      link_note: "deducible: corrobora aku-eminently-qualified-human-concept desde [power-mba] (cross-source)"
    - id: aku-habitos-mas-practica-deliberada-igual-maestria-concept
      link_validation: human-validated
      link_note: "deducible: corrobora aku-eminently-qualified-human-concept desde [james-clear] (cross-source)"
  constrained_by: []
  breaks_when: []
  illustrates: []
  challenges: []

content_validation:
  status: llm-authored
  reviewed_by: ""
  review_date: ""
  review_notes: ""

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

taku_relations:
  complementary: []
  alternative_to: []
  precedes: []
  follows: []

created: 2026-06-05
updated: 2026-06-05
status: draft
status_note: ""
---

## Summary

Sistema de autoevaluación de Jocko Willink para medir y dirigir el progreso hacia el Eminently Qualified Human. Operacionaliza The Code traduciéndolo a seis categorías con atributos puntuables de 0 a 5 contra la propia capacidad.

## Core Components

Seis categorías, cada una con atributos:

1. **Health** — Physical Fitness · Sleep/Rest · Diet/Nutrition.
2. **Personal Development** — Intellectual Fitness · Time Management · Financial Management · Personal Goals.
3. **Professional Development** — Performance · Advancement/Qualifications · significado en el trabajo.
4. **Character/Leadership** — Humility · Emotional Control · Mentorship/Charity.
5. **Relationship** — tiempo de calidad con familia · con amigos/colegas.
6. **Preparedness/Safety** — Martial Arts · Weapons Training · Fire/Safety/Emergency · Neighborhood/Community.

Escala por atributo: 0 (inacción/contraproducente) · 1 (mínimo) · 2-4 (esfuerzo serio) · 5 (máximo respecto a la capacidad actual).

## How to Apply

1. Para cada atributo, valora con honestidad brutal tu esfuerzo frente a tu capacidad (you vs you).
2. Asigna 0-5 según los anclajes de la rúbrica de cada categoría.
3. Hazlo a diario; identifica la categoría más floja y actúa sobre ella.
4. Sube el listón a medida que tu capacidad crece (un 5 de hoy será un 1 mañana).
5. Cuando caes del Path, usa los Protocolos para volver.

## Underlying Claims

Se apoya en que el objetivo (EQH) es un camino sin fin, en que la autoevaluación debe ser honesta y humilde, y en que mejorar es cada vez más difícil al crecer la capacidad.

## Strengths

Convierte un ideal difuso en métricas accionables y diarias; cubre la vida completa, no solo lo profesional; relativa a la capacidad propia, lo que la hace justa a cualquier nivel de partida.

## Limitations and Criticisms

Depende por completo de la honestidad del evaluador (sin auditor externo); la escala relativa a la capacidad es subjetiva; el estándar casi imposible puede desmotivar si se confunde el fin (esfuerzo) con la meta (puntuación perfecta).

## Variants and Extensions

Personalizable: las categorías y el Código pueden adaptarse al contexto de cada persona. Formularios descargables en jockopublishing.com/downloads.

## Relaciones

**justified_by** ← [[aku-the-evaluation-concept]] · [[aku-evaluation-scoring-0-5-method]] · [[aku-the-code-concept]] · [[aku-eminently-qualified-human-concept]] · [[aku-autoevaluacion-cuerpo-mente-alma-concept]] · [[aku-sistemas-vs-metas-en-atomic-habits-concept]] · [[aku-claridad-objetivos-fortalezas-exito-claim]] · [[aku-tres-capas-liderazgo-concept]] · [[aku-objetivos-personales-concept]] · [[aku-habitos-mas-practica-deliberada-igual-maestria-concept]]
