---
type: aku
aku_class: concept
id: aku-whisper-tease-shout-concept
statement: >
  El método whisper-tease-shout estructura un lanzamiento (la mejor forma de
  activar afiliados, y de lanzar cualquier cosa): whisper (como los call-outs,
  crear curiosidad mostrando el «entre bastidores» y empezando con semanas o
  años de antelación —cuanto más tarda algo, más lo valora la audiencia), tease
  (revelar el producto y el valor con el framework What-Who-When y hacer pública
  la fecha) y shout (la apertura y venta el día del lanzamiento).
origin: "Alex Hormozi — $100M Leads"
domain: [marketing-digital, lanzamientos, afiliados, hormozi]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-what-who-when-framework-concept
    - aku-content-hook-concept
    - aku-construir-affiliate-army-seis-pasos-concept
    - aku-oferta-al-afiliado-concept
    - aku-cualificar-afiliado-cliente-experto-concept
    - aku-launch-then-integrate-afiliados-concept
    - aku-super-afiliado-niveles-concept

sources:
  - raw/libros/hormozi/100m-leads/100m-leads.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**related** ↔ [[aku-what-who-when-framework-concept]] · [[aku-content-hook-concept]] · [[aku-construir-affiliate-army-seis-pasos-concept]] · [[aku-oferta-al-afiliado-concept]] · [[aku-cualificar-afiliado-cliente-experto-concept]] · [[aku-launch-then-integrate-afiliados-concept]] · [[aku-super-afiliado-niveles-concept]]
