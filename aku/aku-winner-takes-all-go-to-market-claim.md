---
type: aku
aku_class: claim
id: aku-winner-takes-all-go-to-market-claim
statement: >
  En mercados con network effects el ganador se lo lleva todo: el primero en
  alcanzar la masa crítica captura el mercado y bloquea el liderazgo, y disponer
  de una estrategia go-to-market única (un motor de crecimiento identificado que
  permite llegar muy rápido a esa masa crítica) es lo que hace que merezca la pena
  correr los riesgos del blitzscaling.
origin: "Reid Hoffman & Chris Yeh — «Blitzscaling»; The Power MBA — Módulo 4.5"
domain: [estrategia, crecimiento, blitzscaling, escalado, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by:
    - aku-blitzscaling-concept
  supports: []
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related:
    - aku-blitzscaling-cuando-oportunidad-enorme-claim
    - aku-network-effect-concept
    - aku-masa-critica-concept
    - aku-ventaja-competitiva-concept
    - aku-free-bootstrap-plataforma-claim
    - aku-limitadores-crecimiento-blitzscaling-concept

sources:
  - raw/cursos/power-mba/4-5-blitzscaling/4-5-blitzscaling.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**supported_by** ← [[aku-blitzscaling-concept]]
**related** ↔ [[aku-blitzscaling-cuando-oportunidad-enorme-claim]] · [[aku-network-effect-concept]] · [[aku-masa-critica-concept]] · [[aku-ventaja-competitiva-concept]] · [[aku-free-bootstrap-plataforma-claim]] · [[aku-limitadores-crecimiento-blitzscaling-concept]]
