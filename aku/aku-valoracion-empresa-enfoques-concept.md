---
type: aku
aku_class: concept
id: aku-valoracion-empresa-enfoques-concept
statement: >
  La valoración de una empresa puede abordarse desde tres enfoques: a partir del valor
  de sus activos, a partir del valor de mercado de empresas comparables, y a partir de
  los cash flows que va a generar; incluye que cada enfoque da lugar a métodos
  concretos (valoración por activos, por múltiplos y por descuento de flujos);
  implica que la elección depende de la información disponible y del sector; excluye
  considerar un único método como universalmente correcto.
origin: "The Power MBA — Módulo 9.3 «Conceptos clave: Finanzas»"
domain: [finanzas, valoracion, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: [aku-valoracion-por-activos-method, aku-valoracion-por-multiplos-method, aku-dcf-method]
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related: [aku-enterprise-value-concept]

sources:
  - raw/cursos/power-mba/09-03-01-conceptosclavefinanzas2/09-03-01-conceptosclavefinanzas2.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-valoracion-por-activos-method]] · [[aku-valoracion-por-multiplos-method]] · [[aku-dcf-method]]
**related** ↔ [[aku-enterprise-value-concept]]
