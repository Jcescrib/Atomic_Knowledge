---
type: aku
aku_class: concept
id: aku-fases-financiacion-concept
statement: >
  Las fases de financiación de una startup se alinean con su fase de desarrollo de
  producto: Seed (durante el problem-solution fit) → Early stage (durante el
  product-market fit) → Growth (con el motor de crecimiento en marcha) → Expansion
  (al escalar); implica que cada fase admite fuentes distintas (seed:
  bootstrap/FFF/incubadora/business angel; early: + aceleradora/venture capital;
  growth: + media 4 equity; expansion: + socio industrial) y que acudir a ciertos
  inversores antes de la fase adecuada (p. ej. a un VC en seed) no funciona.
origin: "The Power MBA — Módulo 6.3 «Fuentes de financiación: quién»"
domain: [emprendimiento, financiacion, startup, fases, power-mba]

llm_confidence: 0.50

human_certainty:
  status: unvalidated
  iterations: 0
  context_boundary: ""
  validated_by: ""
  validation_date: ""
  method: ""

epistemic_type: sourced

relations:
  supported_by: []
  supports: [aku-fuentes-financiacion-concept]
  constrained_by: []
  constrains: []
  context_breaks_at: []
  breaks_context_of: []
  contradicts: []
  related: [aku-problem-solution-fit-concept, aku-product-market-fit-concept, aku-motor-crecimiento-concept, aku-business-angel-concept]

sources:
  - raw/cursos/power-mba/6-3-01-fuentes-financiacion-quien-ok/6-3-01-fuentes-financiacion-quien-ok.md

created: 2026-06-04
updated: 2026-06-04
status: active
status_note: ""
---

## Relaciones

**supports** → [[aku-fuentes-financiacion-concept]]
**related** ↔ [[aku-problem-solution-fit-concept]] · [[aku-product-market-fit-concept]] · [[aku-motor-crecimiento-concept]] · [[aku-business-angel-concept]]
